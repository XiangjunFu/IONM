...
public class AddFrameWorkList extends DialogBox {
	...
	interface AddWorkListDialogBoxUiBinder extends
			UiBinder<Widget, AddFrameWorkList> {
	}

	//创建配置工单时
	public AddFrameWorkList() {
	...
	////添加Ifdt时创建配置工单时构造函数
	public AddFrameWorkList(String deviceCode){
		setWidget(uiBinder.createAndBindUi(this));
	...
	public void onSaveClick(ClickEvent event) {
		...
		frameTask.setCode(tempWorkListCode);
		frameTask.setName(tempWorkListName);
		frameTask.setCreatetime(new Date());
		frameTask.setDeviceCode(this.deviceCode.getText());
		frameTask.setDeviceName(this.deviceName.getText());
		frameTask.setDeviceType(this.deviceType2.getText());
		frameTask.setStatus(1);//预配置
		frameTask.setDeadline(deadline.getValue());
		
		frameTask.setFrameNum(frameNum.getText());
		boolean optype = opType.getValue().booleanValue();
		frameTask.setOpType(optype ? "1":"2");
		
		frameWorkListService.addFrameTask(frameTask,new AsyncCallback<Integer>() {
			@Override
			public void onFailure(Throwable caught) {
				Window.alert("保存失败！！！");
			}
