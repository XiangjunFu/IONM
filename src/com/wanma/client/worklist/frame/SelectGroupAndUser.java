...
public class SelectGroupAndUser extends DialogBox {
	...
	private FrameWorkListManagerTable workListManagerTable = null;
	
	public SelectGroupAndUser(String workListCode,FrameWorkListManagerTable workListManagerTable) {
	...
	void onAssignClick(ClickEvent event){
		...
		frameWorkListService.assignFrameTask(workListCode,groupName,userName,assignTask);
	}
	...
	private AsyncCallback<Boolean> assignTask = new AsyncCallback<Boolean>() {
		...
		public void onSuccess(Boolean result) {
			if(result.booleanValue()){
				hide();
				//刷新表格显示全部工单
				workListManagerTable.updateFrameWorkList();
			}else{
				Window.alert("失败！！！");
			}
		}
