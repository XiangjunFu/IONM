...
public class SelectGroupAndUser extends DialogBox {
	...
	private CheckWorkListManagerTable workListManagerTable = null;
	
	public SelectGroupAndUser(String workListCode,CheckWorkListManagerTable workListManagerTable) {
	...
	void onAssignClick(ClickEvent event){
		...
		checkWorkListService.assignCheckTask(workListCode,groupName,userName,assignTask);
	}
	...
	private AsyncCallback<Boolean> assignTask = new AsyncCallback<Boolean>() {
		...
		public void onSuccess(Boolean result) {
			if(result.booleanValue()){
				hide();
				//刷新表格显示全部工单
				workListManagerTable.updateCheckWorkList();
			}else{
				Window.alert("失败！！！");
			}
		}
