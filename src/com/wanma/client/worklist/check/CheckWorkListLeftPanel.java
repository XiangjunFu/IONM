...
public class CheckWorkListLeftPanel extends Composite {
	...
	interface WorkListLeftPanelUiBinder extends
			UiBinder<Widget, CheckWorkListLeftPanel> {
	}
	...
	public TreePanel<ModelData> getTreePanel2(Folder model) {
		...
		MenuItem terminalUnit = new MenuItem("巡检工单",
				new SelectionListener<MenuEvent>() {
					...
					public void componentSelected(MenuEvent ce) {
						...
						if("巡检工单管理".equals(deviceType)){
							return;
						}
						checkWorkListService.getAllCheckWorkList(deviceType,new AsyncCallback<List<RouteCheckTask>>() {
							@Override
							...
							public void onSuccess(List<RouteCheckTask> result) {
								if(result != null){
									checkWorkListManagerTable.setCheckWorkList(deviceType,result);
								}else{
		...
		MenuItem addCheckWorkList = new MenuItem("增加巡检工单",new SelectionListener<MenuEvent>() {
			@Override
			public void componentSelected(MenuEvent ce) {
				//选择要添加工单的设备以及类型（未配置的设备），填写（或者自动生成）工单信息（就是设备基本信息）
				//保存
				//ModelData item = tree.getSelectionModel().getSelectedItem();
				AddCheckWorkList addWorkList = new AddCheckWorkList();
				addWorkList.center();
				addWorkList.show();
			}
		});
		contextMenu.add(addCheckWorkList);
	...
	public Folder getTreeModel(){
		...
		deviceTypes = new Folder[]{new Folder("巡检工单管理",types)};
