...
public class AddCheckWorkList extends DialogBox {
	...
	private CheckWorkListServiceAsync checkWorkListService = GWT.create(CheckWorkListService.class);
	
	interface AddWorkListDialogBoxUiBinder extends
			UiBinder<Widget, AddCheckWorkList> {
	}

	//创建配置工单时
	public AddCheckWorkList() {
	...
	////添加Ifdt时创建配置工单时构造函数
	public AddCheckWorkList(String deviceCode){
		setWidget(uiBinder.createAndBindUi(this));
	...
	public void onSaveClick(ClickEvent event) {
		...
		RouteCheckTask checkTask = new RouteCheckTask();
		checkTask.setCode(tempWorkListCode);
		checkTask.setName(tempWorkListName);
		checkTask.setCreateTime(new Date());
		checkTask.setDeviceCode(this.deviceCode.getText());
		checkTask.setDeviceName(this.deviceName.getText());
		checkTask.setDeviceType(this.deviceType2.getText());
		checkTask.setStatus((short) 1);//预配置
		checkTask.setStation(stationCode.getText());
		checkTask.setHostroom(hostRoomCode.getText());
		
		checkWorkListService.addRouteCheckTask(checkTask,new AsyncCallback<String>() {
			@Override
			public void onFailure(Throwable caught) {
				Window.alert("保存失败！！！");
			}
