...
public class BoardWorkListLeftPanel extends Composite {
	...
	public TreePanel<ModelData> getTreePanel2(Folder model) {
		...
		MenuItem terminalUnit = new MenuItem("工单",
				new SelectionListener<MenuEvent>() {
					...
					public void componentSelected(MenuEvent ce) {
						...
						if("板卡工单管理".equals(deviceType)){
							return;
						}
						boardWorkListService.getAllBoardWorkList(deviceType,new AsyncCallback<List<BoardTask>>() {
							@Override
							...
							public void onSuccess(List<BoardTask> result) {
								if(result != null){
									boardWorkListManagerTable.setBoardWorkList(deviceType,result);
								}else{
		...
		MenuItem addBoardWorkList = new MenuItem("增加板卡",new SelectionListener<MenuEvent>() {
			@Override
			public void componentSelected(MenuEvent ce) {
				//选择要添加工单的设备以及类型（未配置的设备），填写（或者自动生成）工单信息（就是设备基本信息）
				//保存
				//ModelData item = tree.getSelectionModel().getSelectedItem();
				AddBoardWorkList addWorkList = new AddBoardWorkList();
				addWorkList.center();
				addWorkList.show();
			}
		});
		contextMenu.add(addBoardWorkList);
	...
	public Folder getTreeModel(){
		...
		deviceTypes = new Folder[]{new Folder("板卡工单管理",types)};
