...
public class SelectGroupAndUser extends DialogBox {
	...
	private BoardWorkListManagerTable workListManagerTable = null;
	
	public SelectGroupAndUser(String workListCode,BoardWorkListManagerTable workListManagerTable) {
	...
	void onAssignClick(ClickEvent event){
		...
		boardWorkListService.assignBoardTask(workListCode,groupName,userName,assignTask);
	}
	...
	private AsyncCallback<Boolean> assignTask = new AsyncCallback<Boolean>() {
		...
		public void onSuccess(Boolean result) {
			if(result.booleanValue()){
				hide();
				//刷新表格显示全部工单
				workListManagerTable.updateBoardWorkList();
			}else{
				Window.alert("失败！！！");
			}
		}
